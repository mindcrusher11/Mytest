...
crawl_player = db.crawl_by_birth_info_insert
element_list = []
...
def get_player_url(url,state_location):
	try:
		...
		time.sleep(60)
		get_player_url(url,state_location)
...
def get_children_rows(all_rows,url,state_location):
		...
		if child_count == 3:
			...
			dob = child_list[1].string
			city = child_list[2].string
			...
			#crawl_player.update({'name':player_name},{'$set':{'dob' : dob,'city':city,'state':state,'player_url':player_url,'state_url':state_url}},True)
			crawl_player.insert({'dob' : dob,'city':city,'state':state,'player_url':player_url,'state_url':state_url})

getCityUrls("http://www.basketball-reference.com/friv/birthplaces.cgi")
#getCityUrls()
